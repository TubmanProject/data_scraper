"""Pipeline for a denormalized filings view."""

filings_denormalized_pipeline = [
    {
        "$lookup": {
    ...
    {
        "$lookup": {
            "from": "defendant_birthplace",
            "let": {"def_birthplace": "$defendant_birthplace_code"},
            "pipeline": [
                {
                    "$match": {
                        "$expr": {
                            "$eq": ["$code", "$$def_birthplace"]
                        }
                    }
                },
                {
            ...
            "as": "defendant_birthplace"
        }
    },
    {
        "$unwind": "$defendant_birthplace"
    },
    {
        "$lookup": {
            "from": "defendant_uscitizen",
            "let": {"def_uscitizen": "$defendant_uscitizen_code"},
            "pipeline": [
                {
                    "$match": {
                        "$expr": {
                            "$eq": ["$code", "$$def_uscitizen"]
                        }
                    }
                },
                {
            ...
            "as": "defendant_uscitizen"
        }
    },
    {
        "$unwind": "$defendant_uscitizen"
    },
    {
        "$project": {
            "_id": 0,
            ...
            "defendant": {
                ...
                "birthplace": "$defendant_birthplace",
                "us_citizen": "$defenant_uscitizen"
            },
            "attorney": {
                "name": "$attorney_name",
                "spn": "$attorney_spn",

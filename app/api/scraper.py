...
@scraper.route('/filing/today', methods=['POST'])
def scrape_filing_today():
    
    today = datetime.datetime.today()
    
    # download today's filing report
    try:
        Filings.download_filing_report_by_date(today)
    except Exception as err:
        if err.__class__.__name__ == 'HTTPError':
            return Response.make_error_resp(msg=err.response.reason, type=err.__class__.__name__, code=err.response.status_code)
        else:
    ...
    # TODO: fix this
    # wait for the file to be downloaded
    filename = '%s_criminal_filing.txt'%(today.strftime('%Y-%m-%d'))
    ...
    # parse the downloaded disposition report
    try: 
       parsed = Filings.parse_filing_data_by_date(today)
    except Exception as err:
        return Response.make_error_resp(msg=str(err), type=err.__class__.__name__, code=500) 
    finally:
        if parsed:
            return Response.make_success_resp(msg="Filing report for today has been saved to the database.")
        else: 
            return Response.make_error_resp(msg="Error: Unknown error parsing the filing data file", type="Error", code=500)

@scraper.route('/filing/seed_db', methods=['POST'])
def seed_filing_db():
     """
     This endpoint doesn't work.
     The HCDC public dataset website doesn't seem to archive filing data
     """
     try:
        num_found = Filings.seed_db()
     except Exception as err:
        if err.__class__.__name__ == 'HTTPError':
            return Response.make_error_resp(msg=err.response.reason, type=err.__class__.__name__, code=err.response.status_code)
        else:
         ...
         if num_found:
            return Response.make_success_resp(msg="%s filing reports have been added to the database."%(num_found))
         else: 
            return Response.make_error_resp(msg="Error: Unknown error seeding the filing database.", type="Error", code=500)